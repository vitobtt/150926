...
import { SHADERS_MAP } from "@/components/ui/cards-shader-effect-utils/shaders-map";
...
export function CardsShader() {
  ...
  // Track hover state natively to avoid React enclosure delays/stales in 60fps tick
  const isHoveredRef = useRef<boolean>(false);
  const [designIndex, setDesignIndex] = useState<number>(0);

  // Smoothly interpolated flip rotation angle around Y axis
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // Calculate Card Metrics (shrink cards if height is small to save vertical space)
  ...
  const renderLoop = () => {
    ...
    // Smoothly interpolate the Y rotation flip progress
    const targetFlip = isHoveredRef.current ? 180 : 0;
    flipAngle.current += (targetFlip - flipAngle.current) * 0.1;
    ...
    // Flip factor inverts Y-tilt when card is face-down to feel completely natural under user's cursor
    const flipFactor = Math.cos((flipAngle.current * Math.PI) / 180);
    const activeTiltX = -mouse.current.y * maxTiltX;
    const activeTiltY = mouse.current.x * maxTiltY * flipFactor;

    const totalRotX = activeTiltX;
    const totalRotY = flipAngle.current + activeTiltY;

    card.style.zIndex = "100";
    card.style.opacity = "1";
    card.style.visibility = "visible";

    // Inject translation matrix with the premium -2deg tilt combined with dynamic mouse-interactive 3D tilt
    card.style.transform = `translateZ(130px) rotateX(${totalRotX.toFixed(2)}deg) rotateY(${totalRotY.toFixed(2)}deg) rotateZ(-2deg)`;
  };
  ...
  return (
    <div className="absolute inset-0 bg-[#000000] text-white flex items-center justify-center overflow-hidden select-none">
      ...
      <header
        style={{ display: "none" }}
        className="hidden absolute top-0 left-0 right-0 p-5 sm:p-6 lg:p-16 z-50 flex items-center justify-between pointer-events-none"
      >
      ...
      >
        ...
        <div
          ...
          className="absolute cursor-pointer active:scale-[0.97] transition-all duration-300 ease-out pointer-events-auto focus:outline-none"
          style={{
            ...
            >
              ...
              {thicknessLayers.map((zOffset, layerIdx) => {
                ...
                const ShaderComponent =
                  SHADERS_MAP[designIndex % SHADERS_MAP.length];
                const baseBgColor = ShaderComponent
                  ? "#0f0f0f"
                  : CARD_COLORS[designIndex % CARD_COLORS.length];
                ...
                if (isFrontFace) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 p-5 sm:p-6 text-white h-full w-full font-sans z-10">
                        ...
                        <div className="absolute left-5 sm:left-6 top-1/2 -translate-y-1/2">
                          ...
                          >
                            <path
                              ...
                              fill={`url(#paint0_linear_1032_4_${designIndex})`}
                            />
                            <path
                              fillRule="evenodd"
                              clipRule="evenodd"
                            ...
                            <defs>
                              <linearGradient
                                id={`paint0_linear_1032_4_${designIndex}`}
                                x1="30"
                                y1="8"
                                x2="30"
                                y2="52"
                        ...
                        <div className="absolute right-5 sm:right-6 top-5 sm:top-6 opacity-95">
                          <svg
                            ...
                            width="4032"
                            height="1344"
                            viewBox="0 0 4032 1344"
                            fill="none"
                            className="w-[54px] xs:w-[101px] sm:w-[120px] h-auto text-white/40"
                          >
                            <path
                              d="M1344 1344H672V672H1344V1344ZM2688 1344H2016V672H2688V1344ZM4032 1344H3360V672H4032V1344ZM672 672H0V0H672V672ZM2016 672H1344V0H2016V672ZM3360 672H2688V0H3360V672Z"
                              fill="currentColor"
                            />
                          </svg>
                        </div>
                ...
                if (isBackFace) {
                  ...
                  const details =
                    CARD_DETAILS[designIndex % CARD_DETAILS.length];
                  return (
                    <div
                      key={layerIdx}
                    ...
                    >
                      {/* Render WebGL Shader with premium 16px blur on the back face of the card */}
                      {ShaderComponent && (
                        <div
